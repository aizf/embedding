...
class TSEM(Estimator):
    ...
    def fit(self, graph: nx.classes.graph.Graph, T: Union[np.array, coo_matrix]):
        ...
        # x = self._create_base_matrix(graph)
        # print(x, x.shape)
    ...
    def get_embedding(self) -> np.array:
        ...
        """
        # print("U", self._U.shape)
        # print("Q", self._Q.shape)
        embedding = np.concatenate([self._U, self._Q], axis=1)
        return embedding, self._M, self._V
