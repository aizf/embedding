"""Parser tool and helper functions."""
...
def parameter_parser():
    ...
    parser.add_argument(
        "--edge-path",
        nargs="?",
        default="./input/" + dataset + "_edges.csv",
        help="Edge list csv.")

    parser.add_argument(
        "--feature-path",
        nargs="?",
        default="./input/" + dataset + "_features.json",
        # default="./input/" + dataset + "_features.csv",
        help="Node features csv.")

    parser.add_argument(
        "--output-path",
        nargs="?",
        default="./output/" + dataset + "_tene.csv",
...
def normalize_adjacency(graph):
    ...
    degs = sparse.csr_matrix(
        sparse.coo_matrix((degs, (ind, ind)), shape=A.shape, dtype=np.float32))
    A = A.dot(degs)
    return A
...
def read_sparse_features(feature_path):
    ...
    features = sparse.csr_matrix(
        sparse.coo_matrix((values, (index_1, index_2)),
